...
def generate_pa_network(year, gamma=gamma,
    ...
    if to_save:
        serialize('deg_dist/{0}_pa_{1}.pkl.gz'.format(year, gamma), deg_dist)
        serialize('delay_dist/{0}_pa_{1}.pkl.gz'.format(year, gamma), delay_dist)
...
'''
def search_for_params(year, min_gamma, max_gamma, inc_gamma):
    fout = open('results/{0}_pa_results_{1}.csv'.format(year, min_gamma), 'w')

    gammas = frange(min_gamma, max_gamma, inc_gamma)

    for gamma in gammas:
        print(datetime.now())
        deg_tss, delay_tss = generate_pa_network(
            year, gamma, to_save=True, compare=True)
        print('gamma={1}\ndegree TSS={2}\ndelay TSS={3}\ncombined TSS={4}\n'.format(0, gamma, deg_tss, delay_tss, deg_tss + delay_tss))
        fout.write('{1},{2},{3},{4}\n'.format(
                0, gamma, deg_tss, delay_tss, deg_tss + delay_tss))
...
def test_inputs():
    while True:
        ...
        deg_tss, delay_tss = generate_pa_network(
            '2003', cur_gamma, plot=True, compare=True)
